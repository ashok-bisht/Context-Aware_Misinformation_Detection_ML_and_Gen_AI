...
st.caption("AWS S3 \u00b7 XGBoost (TF-IDF) \u00b7 DistilBERT (RAG retrieval) \u00b7 VADER sentiment \u00b7 Groq (free LLM) \u00b7 Dashboard")
...
with st.sidebar:
    ...
    vectorizer_key = st.text_input("Vectorizer key", value="models-pkl-file/tfidf_vectorizer.pkl")
    model_key = st.text_input("Model key", value="models-pkl-file/XGBoost (1).pkl")
    ...
    if not st.session_state.last_prediction:
        ...
        if st.button("Generate Explanation"):
            with st.spinner("Retrieving context and generating explanation..."):
                try:
                    # kb_vectorizer here is the DistilBERT SentenceTransformer model
                    # and kb_matrix is a dense array of embeddings (see
                    # utils/rag_engine.py) -- retrieval now runs on semantic
                    # similarity instead of TF-IDF cosine similarity, but the
                    # call signature is unchanged so nothing else below differs.
